...
Static list of upcoming matches. Used by the dashboard's points table to surface each team's next match.

| Column       | Type                | Description                            |
| ------------ | ------------------- | -------------------------------------- |
| match_number | int                 | Continues from `matches.csv` numbering |
| date         | string (YYYY-MM-DD) | Scheduled match date                   |
| venue        | string              | Venue name                             |
| team_1       | string              | Team 1                                 |
| team_2       | string              | Team 2                                 |
...
| is_boundary      | bool         | `True` only when the delivery was a 4 or 6 hit. `False` for every non-4/6 delivery, _and_ `False` for the rare case where the batter physically ran 4 or 6. Use this column to count boundaries                                                                                                                                                                                                      |
| non_boundary_run | bool         | `True` only when `batter_runs ∈ {4, 6}` _and_ the runs came from running, not a boundary. Mirrors cricsheet's `non_boundary` flag. Almost always `False`                                                                                                                                                                                                                                             |
...
### `reviews.csv` _(only present when DRS reviews occurred)_
...
### `super_over.csv` _(only present when a super over was played)_
...
### `substitutions.csv` _(only present when substitutions occurred)_
