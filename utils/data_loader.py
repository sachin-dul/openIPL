...
@lru_cache(maxsize=1)
def load_matches():
    return safe_read_csv(DATA_DIR / "matches.csv")


@lru_cache(maxsize=1)
def load_fixtures():
    """Upcoming scheduled matches (hand-maintained, not generated by orchestrator)."""
    return safe_read_csv(DATA_DIR / "fixtures.csv")


@lru_cache(maxsize=1)
def load_points_table():
    return safe_read_csv(DATA_DIR / "points_table.csv")


@lru_cache(maxsize=1)
def load_players():
    return safe_read_csv(DATA_DIR / "players.csv")


@lru_cache(maxsize=None)
def concat_match_csvs(filename):
...
def load_ball_by_ball():
    df = concat_match_csvs("ball_by_ball.csv")
    if df.empty:
        return df
...
def load_batting_scorecards():
    return concat_match_csvs("batting_scorecard.csv")


@lru_cache(maxsize=1)
def load_bowling_scorecards():
    return concat_match_csvs("bowling_scorecard.csv")


@lru_cache(maxsize=1)
def load_partnerships():
    return concat_match_csvs("partnerships.csv")


@lru_cache(maxsize=1)
def load_fall_of_wickets():
    return concat_match_csvs("fall_of_wickets.csv")


@lru_cache(maxsize=1)
def load_phase_summaries():
    return concat_match_csvs("phase_summary.csv")


@lru_cache(maxsize=1)
def load_reviews():
    return concat_match_csvs("reviews.csv")


@lru_cache(maxsize=1)
def load_substitutions():
    return concat_match_csvs("substitutions.csv")


@lru_cache(maxsize=1)
def load_super_over():
    return concat_match_csvs("super_over.csv")
